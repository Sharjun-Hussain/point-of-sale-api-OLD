...
const buildInvoicePdf = (doc, sale, settings, branchCount = 1) => {
    ...
    // ── HEADER ─────────────────────────────────────────────────────────────────
    let logoHeight = 0;
    if (settings.showLogo && settings.logoFilePath) {
        const logoPath = path.join(process.cwd(), settings.logoFilePath);
        if (fs.existsSync(logoPath)) {
            try {
                doc.image(logoPath, MARGIN, MARGIN, { height: 48, fit: [120, 48] });
                logoHeight = 48;
            } catch (e) { /* ignore */ }
        }
    }

    const companyX = logoHeight > 0 ? MARGIN + 140 : MARGIN;
    
    const maxNameW = CONTENT_WIDTH * 0.55;
    let bizNameSize = 22;
    doc.font('Times-Bold');
    ...
    const metaBoxW = Math.max(Math.ceil(rawInvoiceWidth), 190); // Minimum 190px width to fit long invoice numbers safely
    
    // Draw INVOICE right-aligned to match the box perfectly. Add +10 to width to prevent PDFKit text-wrapping bugs
    doc.text('INVOICE', RIGHT_EDGE - metaBoxW - 10, MARGIN, { width: metaBoxW + 10, align: 'right', characterSpacing: 1 });
